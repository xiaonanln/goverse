...
if %ERRORLEVEL% EQU 0 (
    ...
    echo   Container name: pulse-etcd
    echo   Client endpoint: http://localhost:2379
    echo.
    echo To stop: docker stop pulse-etcd
    echo To remove: docker rm pulse-etcd
) else (
    echo.
    echo ✗ Failed to start etcd container
    echo   Make sure Docker is running
    echo   Or the container might already exist - try: docker rm -f pulse-etcd
)
