...
// durationUs is the execution duration in microseconds.
func (im *InspectorManager) NotifyObjectCall(objectID, objectType, method string, durationUs int32) {
	// If inspector is disabled, skip all work
	if im.inspectorAddress == "" {
		return
	}
	...
	if shouldNotify {
		...
		taskpool.SubmitByKey(objectID, func(ctx context.Context) {
			im.reportObjectCall(ctx, objectID, objectType, method, durationUs)
		})
	}
}
...
// This method is safe to call from background goroutines.
func (im *InspectorManager) reportObjectCall(ctx context.Context, objectID, objectType, method string, durationUs int32) {
	im.mu.RLock()
	client := im.client
	im.mu.RUnlock()
	...
	req := &inspector_pb.ReportObjectCallRequest{
		...
		Method:              method,
		NodeAddress:         im.address,
		ExecutionDurationUs: durationUs,
	}
