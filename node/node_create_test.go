...
func TestCreateObject_RequiresID(t *testing.T) {
	...
	_, err := node.CreateObject(ctx, "TestObject", "", 0)
	if err == nil {
	...
	id, err := node.CreateObject(ctx, "TestObject", "test-obj-123", 0)
	if err != nil {
...
func TestCreateObject_DuplicateID(t *testing.T) {
	...
	// Create first object
	id1, err := node.CreateObject(ctx, "TestObject", "duplicate-id", 0)
	if err != nil {
	...
	id2, err := node.CreateObject(ctx, "TestObject", "duplicate-id", 0)
	if err != nil {
...
func TestCreateObject_DuplicateID_DifferentType(t *testing.T) {
	...
	_, err := node.CreateObject(ctx, "TestObject", "same-id", 0)
	if err != nil {
		t.Fatalf("Failed to create first object: %v", err)
	}

	// Try to create object with same ID but different type - should fail
	_, err = node.CreateObject(ctx, "TestObject2", "same-id", 0)
	if err == nil {
...
func TestCreateObject_UnknownType(t *testing.T) {
	...
	_, err := node.CreateObject(ctx, "UnknownType", "test-obj-456", 0)
	if err == nil {
...
func TestCreateObject_ConcurrentCalls(t *testing.T) {
	...
	for i := 0; i < numGoroutines; i++ {
		go func() {
			id, err := node.CreateObject(ctx, "TestConcurrencyObject", objectID, 0)
			results <- struct {
				id  string
				err error
				obj Object
...
func TestCreateObject_ConcurrentDifferentObjects(t *testing.T) {
	...
	for i := 0; i < numObjects; i++ {
		...
		go func(id string) {
			returnedID, err := node.CreateObject(ctx, "TestConcurrencyObject", id, 0)
			results <- struct {
				id  string
				err error
			}{returnedID, err}
