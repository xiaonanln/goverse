...
func TestClusterReadyRequiresBothConnectionsAndShardMapping(t *testing.T) {
	...
	defer c.UnregisterNodeForTesting(ctx)
	...
	err = c.StartNodeConnectionsForTesting(ctx)
	if err != nil {
		t.Fatalf("Failed to start node connections: %v", err)
	}
	defer c.StopNodeConnectionsForTesting()
	...
	err = c.StartShardMappingManagementForTesting(ctx)
	if err != nil {
		t.Fatalf("Failed to start shard mapping management: %v", err)
	}
	defer c.StopShardMappingManagementForTesting()
