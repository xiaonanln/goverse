...
func TestShardMetricsIntegration(t *testing.T) {	addr1 := testutil.GetFreeAddress()
	...
	metric1 := promtestutil.ToFloat64(metrics.AssignedShardsTotal.WithLabelValues(addr1))
	metric2 := promtestutil.ToFloat64(metrics.AssignedShardsTotal.WithLabelValues(addr2))
