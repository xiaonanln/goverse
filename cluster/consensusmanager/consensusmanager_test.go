...
func TestIsStateStable(t *testing.T) {
	...
	cm := NewConsensusManager(mgr, shardlock.NewShardLockWithShards(testNumShards), 10*time.Second, localAddr, testNumShards)
...
func TestClaimShardOwnership(t *testing.T) {
	...
	cm := NewConsensusManager(mgr, shardlock.NewShardLockWithShards(testNumShards), 10*time.Second, thisNodeAddr, testNumShards)
