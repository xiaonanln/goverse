...
// ClearShardCurrentNode clears the CurrentNode field for a shard after migration
// This signals that the shard is no longer on the old node
func (cm *ConsensusManager) ClearShardCurrentNode(ctx context.Context, shardID int, expectedModRevision int64) error {
if cm.etcdManager == nil {
return fmt.Errorf("etcd manager not set")
}
...
// Create updated shard info with CurrentNode cleared
updatedInfo := ShardInfo{
TargetNode:  shardInfo.TargetNode,
CurrentNode: "", // Clear the current node
ModRevision: expectedModRevision,
}

// Store the updated shard info
prefix := cm.etcdManager.GetPrefix()
key := fmt.Sprintf("%s/shard/%d", prefix, shardID)
value := formatShardInfo(updatedInfo)

// Use conditional transaction to ensure we only update if ModRevision matches
txn := client.Txn(ctx).
...
resp, err := txn.Commit()
if err != nil {
return fmt.Errorf("failed to clear CurrentNode for shard %d: %w", shardID, err)
}

if !resp.Succeeded {
return fmt.Errorf("failed to clear CurrentNode for shard %d: ModRevision mismatch", shardID)
}

cm.logger.Infof("Cleared CurrentNode for shard %d (was %s, target %s)",
