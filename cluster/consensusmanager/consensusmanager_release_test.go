...
func TestReleaseShardsForNode_NoShardMapping(t *testing.T) {
	...
	cm := NewConsensusManager(mgr, shardlock.NewShardLock(testNumShards), 1*time.Second, "localhost:47001", testNumShards)
...
func TestReleaseShardsForNode_WithEtcd(t *testing.T) {
	...
	cm := NewConsensusManager(mgr, shardlock.NewShardLock(testNumShards), 1*time.Second, thisNodeAddr, testNumShards)
	prefix := mgr.GetPrefix()
